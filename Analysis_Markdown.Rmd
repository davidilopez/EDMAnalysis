...
timbre <- read.csv("20140714/timbre20140714 with pilot.csv", header=T)
...
# # Analysis by similarity level H/M/L
# # Prep the data
# # 4-pt scale
# timbre.hi.sim.4 <- desc.timbre.4[desc.timbre.4$mean > 2.8,]
# timbre.low.sim.4 <- desc.timbre.4[desc.timbre.4$mean < 2.2,]
# timbre.mid.sim.4 <- desc.timbre.4[desc.timbre.4$mean >= 2.2 & desc.timbre.4$mean <= 2.8,]
# #2-pt scale
# timbre.hi.sim.2 <- desc.timbre.2[desc.timbre.2$mean >= 1.666,]
# timbre.low.sim.2 <- desc.timbre.2[desc.timbre.2$mean < 1.333,]
# timbre.mid.sim.2 <- desc.timbre.2[desc.timbre.2$mean >= 1.333 & desc.timbre.2$mean <= 1.666,]
# 
# # Prep the data
# # 4-pt scale
# timbre.hi.sim.4 <- desc.timbre.4[desc.timbre.4$mean >= 2.5,]
# timbre.low.sim.4 <- desc.timbre.4[desc.timbre.4$mean < 2.5,]
# #2-pt scale
# timbre.hi.sim.2 <- desc.timbre.2[desc.timbre.2$mean >= 1.5,]
# timbre.low.sim.2 <- desc.timbre.2[desc.timbre.2$mean < 1.5,]
# 
# #Fleiss' Kappa of levels
# # 4-pt scale
# fleiss.timbre.low.sim.4 <- fleiss.EDM(t(clean.timbre.4[rownames(timbre.low.sim.4),]), is.raw = F)
# fleiss.timbre.hi.sim.4 <- fleiss.EDM(t(clean.timbre.4[rownames(timbre.hi.sim.4),]), is.raw = F)
# fleiss.timbre.mid.sim.4 <- fleiss.EDM(t(clean.timbre.4[rownames(timbre.mid.sim.4),]), is.raw = F)
# 
# # 2-pt scale
# fleiss.timbre.low.sim.2 <- fleiss.EDM(t(clean.timbre.2[rownames(timbre.low.sim.2),]), is.raw = F)
# fleiss.timbre.hi.sim.2 <- fleiss.EDM(t(clean.timbre.2[rownames(timbre.hi.sim.2),]), is.raw = F)
# fleiss.timbre.mid.sim.2 <- fleiss.EDM(t(clean.timbre.2[rownames(timbre.mid.sim.2),]), is.raw = F)

# Confidence levels
# 4-pt scale
conf.timbre.4 <- t(confidence.values(reliability.check(timbre), polarize = FALSE))
rownames(conf.timbre.4) <- seq(1:190)
desc.conf.timbre.4 <- descriptives(data = t(conf.timbre.4), raw = F)
ordered.conf.timbre.4 <- conf.timbre.4[order(desc.conf.timbre.4$mean, decreasing = TRUE),]
easy.timbre.4 <- ordered.conf.timbre.4[1:63,]
intermediate.timbre.4 <- ordered.conf.timbre.4[64:126,]
difficult.timbre.4 <- ordered.conf.timbre.4[127:190,]
fleiss.easy.timbre.4 <- fleiss.EDM(data = t(easy.timbre.4), is.raw = F, iterations = 1000)
fleiss.intermediate.timbre.4 <- fleiss.EDM(data = (intermediate.timbre.4), is.raw = F, iterations = 1000)
fleiss.difficult.timbre.4 <- fleiss.EDM(data = t(difficult.timbre.4), is.raw = F, iterations = 1000)

# 2-pt scale
conf.timbre.2 <- confidence.values(reliability.check(timbre), polarize = FALSE)
desc.conf.timbre.2 <- descriptives(data = conf.timbre.2, raw = F)
...
For four-point scale data (original):
``` {r echo=FALSE, warning=FALSE, comment=NA}
table.timbre.general.4
diff.timbre.general.4
```

For two-point scale data (re-scaled):
``` {r echo=FALSE, warning=FALSE, comment=NA}
table.timbre.general.2
...
diff.timbre.rhythm.sel.4 <- table.timbre.rhythm.4[c(7, 17, 21, 39, 47, 53, 59, 62, 94, 111, 119, 149, 151, 176, 178, 184, 188, 190),]
...
diff.timbre.rhythm.sel.2 <- table.timbre.rhythm.2[c(7, 17, 21, 39, 47, 53, 59, 62, 94, 111, 119, 149, 151, 176, 178, 184, 188, 190),]


# Get descriptives of the selections in timbre and rhythm
# 4-pt scale
sel.timbre.4 <- clean.timbre.4[c(7, 17, 21, 39, 47, 53, 59, 62, 94, 111, 119, 149, 151, 176, 178, 184, 188, 190),]
sel.rhythm.4 <- clean.rhythm.4[c(7, 17, 21, 39, 47, 53, 59, 62, 94, 111, 119, 149, 151, 176, 178, 184, 188, 190),]

desc.timbre.sel.4 <- descriptives(data = t(sel.timbre.4), raw = F, tracknames = T, selection = F)
desc.rhythm.sel.4 <- descriptives(data = t(sel.rhythm.4), raw = F, tracknames = T, selection = F)

# 2-pt scale
sel.timbre.2 <- clean.timbre.2[c(7, 17, 21, 39, 47, 53, 59, 62, 94, 111, 119, 149, 151, 176, 178, 184, 188, 190),]
sel.rhythm.2 <- clean.rhythm.2[c(7, 17, 21, 39, 47, 53, 59, 62, 94, 111, 119, 149, 151, 176, 178, 184, 188, 190),]

desc.timbre.sel.2 <- descriptives(data = t(sel.timbre.2), raw = F, tracknames = T, selection = F)
desc.rhythm.sel.2 <- descriptives(data = t(sel.rhythm.2), raw = F, tracknames = T, selection = F)
```

```{r, echo=FALSE}
comparison <- cbind(desc.timbre.sel.4$PairNumber, desc.timbre.sel.4$mean, 0,  table.timbre.general.4[,"W"], table.timbre.general.4[,"p.value"], desc.general.4$mean, 0,  table.rhythm.general.4[,"W"], table.rhythm.general.4[,"p.value"], desc.rhythm.sel.4$mean, 0, 0, 0, 0, 0, 0)
colnames(comparison) <- c("Pair.Number", "Timbre.mean", "Timbre.similarity.level", "Timbre.vs.General.W", "Timbre.vs.General.p", "General.mean", "General.similarity.level", "Rhythm.vs.General.W", "Rhythm.vs.General.p", "Rhythm.mean", "Rhythm.similarity.level", "Lowest.p.value", "Match", "Expected.general.similarity", "Match.with.general.sim.level", "Overrules")

for (i in 1:nrow(comparison)){
        if (comparison[i,2] >= 2.8){
                comparison[i,3] = "High"
        } else if (comparison[i,2] <= 2.2){
                comparison[i,3] = "Low"
        } else if (comparison[i,2] > 2.2 & comparison[i,2] < 2.8){
                comparison[i,3] = "Medium"
        }
}

for (i in 1:nrow(comparison)){
        if (comparison[i,6] >= 2.8){
                comparison[i,7] = "High"
        } else if (comparison[i,6] <= 2.2){
                comparison[i,7] = "Low"
        } else if (comparison[i,6] > 2.2 & comparison[i,6] < 2.8){
                comparison[i,7] = "Medium"
        }
}

for (i in 1:nrow(comparison)){
        if (comparison[i,10] >= 2.8){
                comparison[i,11] = "High"
        } else if (comparison[i,10] <= 2.2){
                comparison[i,11] = "Low"
        } else if (comparison[i,10] > 2.2 & comparison[i,10] < 2.8){
                comparison[i,11] = "Medium"
        }
}
for (i in 1:nrow(comparison)){
        if (comparison[i,5] < 0.05 & comparison[i,5] < comparison[i,9]) {
                comparison[i,12] = "Timbre"
        } else if (comparison[i,9] < 0.05 & comparison[i,9] < comparison[i,5]) {
                comparison[i,12] = "Rhythm"
        } else if (comparison[i,5] > 0.05 & comparison[i,9] > 0.05) {
                comparison[i,12] = "Unknown"
        }
}

for (i in 1:nrow(comparison)){
        if (comparison[i,12] == "Timbre") {
                if (comparison[i,3] == comparison[i,7]) {
                        comparison[i,13] = "Yes"
                }
        } else if (comparison[i,12] == "Rhythm") {
                if (comparison[i,7] == comparison[i,11]) {
                        comparison[i,13] = "Yes"
                }
        }
}
...
for (i in 1:nrow(comparison)){
        if (comparison[i,3] == "High" & comparison[i,11] == "High") {
                comparison[i,14] = "High"
        } else if (comparison[i,3] == "Medium" & comparison[i,11] == "Medium") {
                comparison[i,14] = "Medium"
        } else if (comparison[i,3] == "Low" & comparison[i,11] == "Low") {
                comparison[i,14] = "Low"
        }
}
...
for (i in 1:nrow(comparison)){
        if (comparison[i,14] == comparison[i,7]){
                comparison[i,15] = "Yes"
        } else {
                comparison[i,15] = "No"
        }
...
for (i in 1:nrow(comparison)){
        if (comparison[i,15] == "Yes") {
                comparison[i,16] = "Both"
        } else {
                if ((comparison[i,3] == "Medium" | comparison[i,3] == "Low") & 
                            (comparison[i,11] == "Medium" | comparison[i,11] == "Low")) {
                        comparison[i,16] = "Unknown"
                } else if ((comparison[i,3] == "High") & 
                            (comparison[i,11] == "Medium" | comparison[i,11] == "Low")) {
                        comparison[i,16] = "Timbre"
                } else if ((comparison[i,3] == "Medium" | comparison[i,3] == "Low") & 
                            (comparison[i,11] == "High")) {
                        comparison[i,16] = "Rhythm"
                } else {comparison[i,16] = "None"}
        }
}
write.csv(comparison, "Tables//comparison.csv")
```
...
print(diff.timbre.rhythm.sel.4)
print(diff.timbre.rhythm.sel.4[diff.timbre.rhythm.sel.4[,2] < 0.05,])
...
print(diff.timbre.rhythm.sel.2)
print(diff.timbre.rhythm.sel.2[diff.timbre.rhythm.sel.2[,2] < 0.05,])
```
